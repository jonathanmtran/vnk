  ...
  handleYtSearch(event: any) {
    ...
    fetch("/api/youtube", {
      ...
      .then((jsonResponse) => {
        ...
        this.setState({
          youTubeQueryResults:
            searchResults.results as Array<YouTubeSearchResults>,
        });
      });
  }
      ...
      <>
        <FormControl mt={3}>
          ...
          <Box display="flex" justifyContent="space-between">
            <Input
              type="text"
              name="ytQuery"
              value={this.state.youTubeQuery}
              onChange={this.handleYtQueryChange}
            />
            &nbsp;
            <IconButton
              aria-label="Search YouTube"
              icon={<SearchIcon />}
              onClick={this.handleYtSearch}
            />
          </Box>
        </FormControl>
        ...
        {this.state.youTubeQueryResults.length > 0 ? (
          <SimpleGrid mt={3} columns={1} spacing={1}>
            {this.state.youTubeQueryResults.map(
                ...
                <Card key={video.id} direction={{ base: "column", sm: "row" }}>
                  ...
                  <CardFooter>
                    <ButtonGroup size="sm" spacing="1">
                      <Button onClick={() => this.handlePreview(video)}>
                        Nghe Trước
                      </Button>
                      <Button
                        onClick={() => this.props.onSetYouTubeVideo(video)}
                      >
                        Select
                      </Button>
                    </ButtonGroup>
          ...
          <ModalContent>
            ...
            <ModalFooter>
              <ButtonGroup size="sm" spacing="1">
                <Button
                  onClick={() =>
                    this.props.onSetYouTubeVideo(this.state.previewVideo)
                  }
                >
                  Select
                </Button>
                <Button onClick={() => this.setState({ isPreviewOpen: false })}>
                  Close
                </Button>
              </ButtonGroup>
            </ModalFooter>
